...
class DeviceManagerTestCase(TestCase):
    ...
    def test_update_all_devices(self):
        ...
        # Check the result
        self.assertEqual(
            self.device_manger.all_devices, [1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 12, 13]
        )
    ...
    def test_get_event_string(self):
        ...
        self.assertEqual(
            self.device_manger.get_event_string("Relative", 0x08), "REL_WHEEL"
        )
        self.assertEqual(
            self.device_manger.get_event_string("Absolute", 0x07), "ABS_RUDDER"
        )
    ...
    @mock.patch("inputs.manager.open", mock.mock_open(read_data=MOCK_DEVICE))
    @mock.patch("glob.glob")
    def test_find_special_repeated(
        self, mock_glob, mock_get_char_names, mock_parse_device_path
    ):
...
class DeviceManagerPlatformTestCase(TestCase):
    ...
    def test_find_xinput(self, mock_windll):
        ...
        found_one = "windll.XInput1_4.dll" in str(
