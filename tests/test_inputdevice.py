...
REPR = 'inputs.devices.common.InputDevice("' + KBD_PATH + '")'
...
class InputDeviceTestCase(TestCase):
    ...
    @mock.patch.object(inputs.devices.common.InputDevice, '_set_name')
    ...
    def test_init(self,
        ...
        inputdevice = inputs.devices.common.InputDevice(manager, KBD_PATH)
    ...
    def test_init_no_device_path_at_all(self):
        ...
        with self.assertRaises(inputs.NoDevicePath):
            inputs.devices.common.InputDevice(manager)
        manager.assert_not_called()

    def test_init_device_path_is_none(self):
        """With a device path of None, it has a device path."""
        manager = mock.MagicMock()
        inputs.devices.common.InputDevice._device_path = None
        with self.assertRaises(inputs.NoDevicePath):
            inputs.devices.common.InputDevice(manager)
        del inputs.devices.common.InputDevice._device_path
    ...
    def test_char_path_override(self,
        ...
        manager = mock.MagicMock()
        inputdevice = inputs.devices.common.InputDevice(manager,
                                         KBD_PATH,
    ...
    @mock.patch.object(inputs.devices.common.InputDevice, '_set_name')
    @mock.patch.object(inputs.devices.common.InputDevice,
                       '_do_iter',
    ...
    def test_get_total_read_size_double(self, mock_set_name):
        ...
        manager = mock.MagicMock()
        inputdevice = inputs.devices.common.InputDevice(manager, KBD_PATH, read_size=2)
        mock_set_name.assert_called()

        size = inputdevice._get_total_read_size()
        self.assertEqual(size, inputs.EVENT_SIZE * 2)

        inputdevice = inputs.devices.common.InputDevice(manager, KBD_PATH, read_size=3)
        size = inputdevice._get_total_read_size()
        self.assertEqual(size, inputs.EVENT_SIZE * 3)

        inputdevice = inputs.devices.common.InputDevice(manager, KBD_PATH, read_size=4)
        size = inputdevice._get_total_read_size()
        self.assertEqual(size, inputs.EVENT_SIZE * 4)

        inputdevice = inputs.devices.common.InputDevice(manager, KBD_PATH, read_size=None)
        size = inputdevice._get_total_read_size()
        self.assertEqual(size, inputs.EVENT_SIZE)
    ...
    @mock.patch.object(inputs.devices.common.InputDevice, '_set_name')
    @mock.patch.object(inputs.devices.common.InputDevice, '__iter__',
